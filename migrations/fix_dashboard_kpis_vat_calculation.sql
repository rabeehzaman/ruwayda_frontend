-- ============================================================================
-- FIX: Dashboard KPIs VAT Calculation - Millions Suffix & Opening Balance
-- ============================================================================
-- Issue: get_dashboard_kpis_2025() function has the same bugs as get_vat_return():
--   1. Missing 'M' (millions) suffix handling in bill amounts
--   2. Missing 'K' (thousands) suffix handling
--   3. Opening Balance bills not excluded
--   4. Wrong calculation: uses vat_amount directly instead of (total_bcy - sub_total_bcy)
--
-- Example Bug: "SAR 1.08M" subtotal + "SAR 1,077,213.68" total
--   - Wrong: Uses vat_amount directly, may parse M as 1.08 instead of 1,080,000
--   - Correct: Parse with M/K multipliers, calculate VAT from difference
--
-- Date: 2025-10-13
-- Related: fix_vat_return_millions_suffix.sql (same bug, different function)
-- ============================================================================

-- Drop and recreate the function with proper VAT calculation
...
BEGIN
    ...
    -- FIXED: Use same logic as get_vat_return() with M/K suffix handling
    WITH vat_output AS (
        -- Output VAT from invoices (already calculated in profit_analysis_view_current)
    ...
    vat_input AS (
        -- Input VAT from bills - FIXED: Calculate from total - subtotal with M/K parsing
        SELECT COALESCE(SUM(
            (CASE
...
BEGIN
    ...
    RAISE NOTICE 'Dashboard KPIs Migration Test - October 2025 Net VAT Payable: %', v_net_vat;
    ...
    IF ABS(v_net_vat - v_vat_return_net) > 1 THEN
        RAISE WARNING 'Dashboard KPIs and VAT Return do not match! Difference: %', ABS(v_net_vat - v_vat_return_net);
    ...
    ELSE
        RAISE NOTICE 'Dashboard KPIs VAT calculation appears correct ✓';
        RAISE NOTICE 'Values match VAT Return page ✓';
    END IF;
END $$;
